...
module.exports = ( b, h, L ) => {
    ...
    const AswMin = 0.088 / 100 * b * s; // cm² / m
    ...
    const Pqp = gpp + g + 0.4 * q; // kN / m
    const Pqpcm = Pqp / 100; // kN / cm

    // Flecha elástica        
    const felastica = 5 * Pqpcm * Lcm * Lcm * Lcm * Lcm / ( 384 * Ecs * Ic ) * 10; // mm

    // Flecha imediata

    // Momento fletor da ação quase permanente
    const Mqp = Pqp * L * L / 8; // kNm
    ...
    const resultados = {
        ...
        Mr, Mqpcm, Ie, felastica, fimediata, fdiferida, fTotal, flimite
    }
